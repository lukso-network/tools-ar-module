...
namespace Mediapipe.Unity {
    public class SelfieMaskAnnotation : HierarchicalAnnotation {
        ...
        public void InitScreen() {
            ...
            if (width == 0 || height == 0) {
                //TODO annotation layer rect should not be 0
                width = 1;
                height = 1;
            }
        ...
        public void Draw(byte[] mask, int width, int height, float minAlpha = 0.9f, float maxAlpha = 1.0f) {
            ...
            /*
             for (int k = 0; k < 10; ++k) {
               float v = BitConverter.ToSingle(mask, k * 4);
               Debug.Log(v);
             }

             for (int k = 0; k < 10; ++k) {
               float v = BitConverter.ToSingle(mask, (int)(mask.Length / 4) - 200 + k * 4);
               Debug.Log(v);
             }*/
            ResetAlpha();
                    ...
                    for (var i = 0; i < height; i++) {
                        for (var j = 0; j < width; j++) {
                            ...
                            if (*maskPtr >= threshold) {
                                ...
                                //SetColorAlpha((width -j-1,j),(i,i), alpha);
                                //SetColorAlpha(GetNearestRange(j, wInterval, _texture.width), GetNearestRange(i, hInterval, _texture.height), alpha);
                                SetColorAlpha(GetNearestRange(width - j - 1, wInterval, _texture.width), GetNearestRange(height - i - 1, hInterval, _texture.height), alpha);
                            }
                            maskPtr++;
                        }
                    }
